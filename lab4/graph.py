...
class Graph:
    def __init__(self, dataset, geo=False):
    ...
    def create_adj_matrix(self, dataset, geo):  # format, passare il formato geo o euc
        for i in dataset:
            for j in dataset:
                if i[0] != j[0]:
                    self.adj_matrix[int(i[0])-1][int(j[0])-1] = compute_distance(i, j, geo)  # None if i == j
...
def compute_distance(i, j, geo):

    if geo:
        lat_i = convert_to_rads(i[1])  # u[0] indice u[1] latitudine del nodo u, u[2] long ==> riga u
